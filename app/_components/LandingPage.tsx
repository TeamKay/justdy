...
export default function Hero() {
  return (
    <section className="relative min-h-screen xl:min-h-0 w-full bg-background text-white flex flex-col justify-between items-center px-6 pt-32 pb-12 xl:py-24 overflow-hidden font-sans select-none">
      ...
      {/* HERO MAIN BODY */}
      <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center text-center my-auto xl:my-0 xl:py-12">
        {/* NEW: SUMMER SIGNUP BONUS GLOWING BADGE */}
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="relative group mb-6 px-6 py-3 rounded-full flex flex-col sm:flex-row items-center gap-1 sm:gap-2 bg-linear-to-r from-amber-400/20 via-yellow-500/30 to-amber-400/20 border border-amber-400/40 shadow-[0_0_30px_rgba(234,179,8,0.2)] backdrop-blur-md overflow-hidden before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_center,rgba(250,204,21,0.15)_0%,transparent_70%)]"
        >
          {/* Subtle outer neon ring effect */}
          <div className="absolute -inset-px rounded-full bg-linear-to-r from-blue-400/30 via-amber-400/40 to-purple-400/30 blur-xs opacity-70 group-hover:opacity-100 transition-opacity pointer-events-none" />

          <div className="relative z-10 flex items-center gap-1.5 text-amber-300 font-bold text-sm sm:text-base tracking-wide uppercase">
            <span>🏝️</span>
            SUMMER SIGNUP BONUS!
            <span>🎁</span>
          </div>

          <span className="hidden sm:inline relative z-10 text-amber-200/40 font-light">
            •
          </span>

          <div className="relative z-10 text-white font-medium text-sm sm:text-base flex items-center gap-1">
            Get{" "}
            <span className="text-yellow-300 font-bold underline decoration-amber-400/60">
              20% Off
            </span>{" "}
            Live Tutoring!
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-6xl mx-auto mt-auto xl:mt-12 pt-8 border-t border-white/10 flex flex-col items-center gap-5"
      ></motion.div>
    </section>
  );
}
