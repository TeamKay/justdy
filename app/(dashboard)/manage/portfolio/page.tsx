...
export default function PortfolioTracker() {
  ...
  const cumulativeHoldings = useMemo(() => {
    ...
    // Current Fidelity Positions are authoritative for current holdings.
    // Accounts History remains authoritative for historical activity.
    ...
    const calculated = Array.from(map.values()).filter(
      (holding) =>
        holding.totalShares > 0.00000001 ||
        (holding.tickerOrSymbol?.toUpperCase() === "CASH" &&
