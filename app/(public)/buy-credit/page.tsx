import CreditCardPage from "@/app/_components/CreditCard";
...
export default async function Page() {
  ...
  // 1. Get the most recent paid transaction to find the current package
  const lastTransaction = await prisma.creditTransaction.findFirst({
    where: { userId: session.user.id },
    orderBy: { createdAt: "desc" },
  });

  // 2. Check if they have ever had a paid plan (excluding 'free_credit')
  const hasHistory = await prisma.creditTransaction.findFirst({
    where: {
      userId: session.user.id,
      packageId: { not: "free_credit" },
    },
  });
  ...
  return (
    <div className="min-h-screen bg-background p-8 font-sans">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
        <CreditCardPage
          currentUserPackageId={lastTransaction?.packageId}
          hasHadPaidPlan={!!hasHistory}
        />
      </div>
    </div>
  );
...
// import CreditCard from "@/app/_components/CreditCard";
...
//         {/* Badge Decorator */}
//         <span className="inline-block px-4 mt-5 mb-0 text-xs font-bold tracking-widest text-primary uppercase bg-primary/10 rounded-full">
//           Fuel Your Learning
//         </span>
...
//             Grab credit bundle, experience professional tutoring. No stress—just
//             pure progress.
//           </p>
